...
//
// It is constructed from a centra/core/mcl/llm.Config so it inherits provider
...
func New(cfg mcllm.Config) (*Client, error) {
	...
	// body" failure), so Client.Timeout stays 0 and we govern with (a)
...
// newHTTPRequest builds the POST. Mirrors centra/core/mcl/llm's gateway posture:
...
// (disables reasoning entirely for token-tight mechanical roles).
