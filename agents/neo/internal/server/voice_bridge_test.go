...
func TestVoiceBridgeSubprocessAgainstRealNeoServer(t *testing.T) {
	...
	python := filepath.Clean("../../../../protocol/tools/gideon/voice/.venv/bin/python")
	...
	defer cancel()
	cmd := exec.CommandContext(ctx, python, "../../../../protocol/tools/voice/probe.py", "--url", httpServer.URL, "--conversation", "voice-bridge", "--wav", wav, "--synthesize")
	cmd.Dir = "."
...
func TestVoiceWorkerCrashMidTurnLeavesTextTurnRunning(t *testing.T) {
	...
	cmd := exec.Command(python, "../../../../protocol/tools/voice/probe.py", "--url", httpServer.URL, "--conversation", "voice-crash", "--wav", wav)
	cmd.Dir = "."
	...
	deadTTS.Close()
	cmd = exec.Command(python, "../../../../protocol/tools/voice/probe.py", "--url", httpServer.URL, "--conversation", "voice-dead-tts", "--wav", wav, "--synthesize")
	cmd.Dir = "."
...
func TestVoiceBargeInIsSingleFlightAndCancelsPriorRun(t *testing.T) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 20*time.Second)
	defer cancel()
	cmd := exec.CommandContext(ctx, python, "../../../../protocol/tools/voice/probe.py", "--url", httpServer.URL, "--conversation", "voice-barge", "--wav", wav, "--interrupt-wav", wav, "--interrupt-after", "0.5")
	cmd.Dir = "."
	var stdout, stderr bytes.Buffer
	cmd.Stdout, cmd.Stderr = &stdout, &stderr
	if err := cmd.Run(); err != nil {
