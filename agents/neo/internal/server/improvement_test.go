...
func TestImprovementGovernedChangeCreatesOnlyQuarantinedCandidateAndRollsBack(t *testing.T) {
	e, _ := newImprovementEngine(t, "")
	manifest, err := os.ReadFile(filepath.Join("..", "..", "..", "..", "protocol", "skills", "brainstorming", "SKILL.mtx"))
	if err != nil {
		t.Fatal(err)
	}
	prose, err := os.ReadFile(filepath.Join("..", "..", "..", "..", "protocol", "skills", "brainstorming", "SKILL.md"))
	if err != nil {
		t.Fatal(err)
	}
	proposal := scheduleProposal(t, e.improvement, improvement.Draft{
