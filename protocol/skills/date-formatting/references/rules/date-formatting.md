...
This guide ensures consistent date and time formatting across applications, respecting user locale settings.
...
### From centralized date utilities
...
<Text>{formatDate(item.createdAt, { hideSeconds: true })}</Text>
```
...
```typescript
import { formatDistanceToNow } from '@{scope}/shared/src/utils/dateUtils';

// "2 hours ago", "5 minutes ago"
<Text>{formatDistanceToNow(item.updatedAt)}</Text>
```
...
function MyComponent() {
  ...
  return (
    <Text>{formatDate(date, { hideSeconds: true })}</Text>
  );
}
```
...
<Text size="$bodyMd" color="$textSubdued">
  {item.createdAt
...
### Example 3: History Item

```typescript
import { formatDate } from '@{scope}/shared/src/utils/dateUtils';

<Text size="$bodyMd" color="$textSubdued">
  {formatDate(item.timestamp, { hideSeconds: true })}
</Text>
```
...
Ensure you're using the centralized date utilities instead of native methods. The utility automatically reads from `appLocale.getLocale()`.
