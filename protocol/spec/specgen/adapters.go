...
func buildBrief(wf *Doc) string {
	...
	fmt.Fprintf(&b, "# %s\n\n", name)
	b.WriteString("The single source of truth for how to work in this repo is **`protocol/spec/`** (kvx).\n")
	b.WriteString("This file is a GENERATED pointer to it. Do not hand-edit it; edit `protocol/spec/workflow.kvx` and run `protocol/spec/specgen`.\n\n")
	...
	if active := wf.Str("meta", "active_feature"); active != "" {
		b.WriteString("\n## Active feature\n\n")
		fmt.Fprintf(&b, "`protocol/spec/%s/spec.kvx` — read its `[meta]` status, `[req.*]` acceptance criteria, and the `[task.*]` list (status + wave + requires). Work one task at a time in wave order; update `status` in the kvx as you go.\n", active)
	}
