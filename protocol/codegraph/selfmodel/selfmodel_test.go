...
func TestDistillIsBoundedAndLookupReturnsCompactFragment(t *testing.T) {
	ix := model.NewIndex()
	ix.AddNode(&model.Node{Id: "centra/agents/neo/internal/agent.Agent.Chat", Kind: model.KindMethod, Name: "Chat", File: "agents/neo/internal/agent/agent.go", Sig: "func (a *Agent) Chat(ctx context.Context, input string) error"})
	artifact := Distill(ix, "b3:test", []string{"agents/neo/internal/agent"}, 32)
	...
	if !strings.Contains(fragment, "agents/neo/internal/agent/agent.go") || strings.Contains(fragment, "return ") {
