...
// TestStoreDirAndFileModes proves the trace store creates its directory 0700 and
// every trace file 0600 on a really-created path.
func TestStoreDirAndFileModes(t *testing.T) {
	dir := filepath.Join(t.TempDir(), "traces")
	s := Open(dir)
	s.Record("run-1", Event{Seq: 1, Phase: "neo", Type: "tool.step"})
	s.Flush()
