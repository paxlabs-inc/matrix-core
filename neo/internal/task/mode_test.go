...
// TestStoreDirAndFileModes proves the task ledger creates its directory 0700 and
// every task file 0600 on a really-created path.
func TestStoreDirAndFileModes(t *testing.T) {
	dir := filepath.Join(t.TempDir(), "tasks")
	s := Open(dir)
	s.Begin("conv-1", "run-1", "seal the vault")
