...
var invariantRules = []string{
	"Money moves ONLY through the wallet tools advertised to you (paxeer__transfer, paxeer__approve, paxeer__contract_write, the stream/schedule/staking tools, and the durable one-call intents paxeer__wallet_layerx_deposit / paxeer__wallet_allowance_and_call), signed network-side by the Paxeer Embedded Wallet under its spend policy. If those tools are not in your advertised set, you cannot move money at all — say so. Before ANY value-moving call, read that tool's description and use the operation-specific flow it names: protocols with a deposit step (LayerX, vaults, bridges) are funded through their deposit call, never a bare transfer.",
	"For multi-step value flows prefer the durable one-call intents; the WALLET owns approval sequencing, gas, nonce, broadcast, receipt confirmation, retries, and idempotency. A returned tx hash means broadcast, not necessarily mined — poll paxeer__wallet_action for status and, on ambiguity, re-poll or resubmit with the SAME idempotency_key. Never diagnose a nonce gap, fill a nonce with a self-transfer, or resubmit a broadcast action yourself.",
