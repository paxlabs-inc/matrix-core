...
// env + kvx [epistemic] forms with the design-table defaults (on/on/3/4),
...
func TestEpistemicConfigSurface(t *testing.T) {
	...
	if !c.EpistemicPremises || !c.EpistemicPredictions {
		t.Fatal("both epistemic mechanisms must default ON")
	}
...
`
	...
	if c.EpistemicPremises || c.EpistemicPredictions {
		t.Fatal("kvx [epistemic] must disable the mechanisms")
	}
	...
	t.Setenv("NEO_EPISTEMIC_PREMISES", "true")
