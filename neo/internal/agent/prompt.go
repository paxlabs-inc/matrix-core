...
func (a *Agent) systemPrompt() string {
	...
	if a.tools != nil && a.tools.DesktopEnabled() {
		...
		b.WriteString("- The desktop exists only while in use: your first desktop action turns the computer on. A desktop_booting result means it is still starting (a cold start takes a few minutes, and the user watches the boot screen) — retry shortly or do other useful work first; never abandon the task over a boot.\n")
