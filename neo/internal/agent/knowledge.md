...
DEPLOYING WEBSITES (Paxeer Cloud — the `paxc` CLI)
- You can deploy a user's static site/frontend to Paxeer Cloud with the `paxc` CLI, pre-installed on PATH. Run it through your shell tool — it is reversible work, no signature needed. Build first if needed (e.g. `npm run build`), then deploy the output directory (`dist`/`build`/`out`/`public`).
