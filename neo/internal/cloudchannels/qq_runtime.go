...
func (g *QQGateway) serve(ctx context.Context, endpoint string, config QQConfig) error {
	...
	if config.SessionID != "" && config.Sequence > 0 {
		...
		err = writer.JSON(map[string]any{"op": 2, "d": map[string]any{"token": "QQBot " + token, "intents": qqIntents, "shard": []int{0, 1}, "properties": map[string]string{"$os": "linux", "$browser": "matrix-neo", "$device": "matrix-neo"}}})
	}
	if err != nil {
		return err
	}
