...
func TestGolden_ParsedShape(t *testing.T) {
	...
	snap := ix.Node("matrix/cortex.Snapshot")
	if snap == nil {
	...
	if got := ix.Forward("matrix/cortex.Snapshot", model.EdgeImplements); len(got) != 1 || got[0] != "matrix/cortex.Persistable" {
		t.Fatalf("Snapshot implements = %v", got)
	}
	if got := ix.Reverse("matrix/cortex.Snapshot", model.EdgeReferences); len(got) != 1 || got[0] != "matrix/cortex.NewStore" {
