...
function isLocalWorkforceProxy(request: NextRequest): boolean {
  return (
    process.env.NODE_ENV !== 'production' &&
    process.env.WORKFORCE_LOCAL_PROXY_ENABLED === '1' &&
    ['127.0.0.1', 'localhost', '::1'].includes(request.nextUrl.hostname) &&
    request.nextUrl.pathname.startsWith('/api/workforce/')
  )
}
...
export async function proxy(request: NextRequest) {
  ...
  const bypassIntl =
    ...
    // Vercel platform endpoints (insights/analytics) are not pages —
    // a locale redirect turns /_vercel/insights/script.js into
    // /en/_vercel/... which serves an HTML 404.
    request.nextUrl.pathname.startsWith('/_vercel') ||
    // The LayerX proxy path is a rewrite (next.config.mjs), not a page —
    // a locale redirect would send it to /en/layerx-api/* which matches
    // nothing.
    request.nextUrl.pathname.startsWith('/layerx-api/') ||
    ...
    // Any static file served from /public (self-model/graph.json,
    // manifest.webmanifest, icons, fonts…) lives at the URL root: the
    // localePrefix:'always' 307 would rewrite it to /en/<file>, which is
    // neither a route nor a file → 404. A file extension in the final
    // segment marks it as an asset, never a page.
  ...
  if (!isPublicPath(innerPath) && !isLocalWorkforceProxy(request)) {
