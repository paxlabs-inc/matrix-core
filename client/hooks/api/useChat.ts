...
export interface NeoTask {
  ...
  searches: NeoSearch[]
  /** Structured finance tool results rendered with the /finance components. */
  finance?: NeoFinanceEvent[]
...
function financeFromFields(fields: Record<string, unknown>, ts?: string): NeoFinanceEvent | null {
  const id = asString(fields.id)
  const tool = asString(fields.tool)
  const raw = fields.payload
  if (!id || !tool || !raw || typeof raw !== 'object' || Array.isArray(raw)) return null
  return {
    id,
    tool,
...
function upsertFinance(
  events: NeoFinanceEvent[] | undefined,
...
export function buildTaskFromTrace(
  ...
  for (const ev of ordered) {
    ...
    switch (ev.type) {
      ...
      case 'tool.finance': {
        const finance = financeFromFields(fields, ev.ts)
        if (finance) task = { ...task, finance: upsertFinance(task.finance, finance) }
        break
      }
      case 'tool.todo': {
    ...
    (
      ...
      subRef.current = subscribeEvents({
        ...
        onUpdate: (u: SSEUpdate) => {
          ...
          // Structured market work → the finance screen on Neo's Computer.
          // Upsert by tool-call id so reconnect replay cannot duplicate it.
          if (ev.type === 'tool.finance') {
            const finance = financeFromFields(ev.fields ?? {}, ev.ts)
            if (finance) {
              setTask((prev) =>
                prev ? { ...prev, finance: upsertFinance(prev.finance, finance) } : prev,
              )
            }
            return
          }
