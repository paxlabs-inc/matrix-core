...
/** The compact sidebar intentionally shows only the newest active tasks. The
 * complete durable list remains available from the All tasks surface. */
export function recentActiveConversations(
  conversations: ConversationSummary[],
  limit = 3,
): ConversationSummary[] {
  return conversations.filter((conversation) => !conversation.archived).slice(0, Math.max(0, limit))
}
