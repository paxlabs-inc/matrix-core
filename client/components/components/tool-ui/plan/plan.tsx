...
import { Loader2, Check, X, MoreHorizontal, ChevronRight } from '@/lib/matrix-icons'
...
const TodoIcon = memo(function TodoIcon({ status }: { status: PlanTodoStatus }) {
  ...
  if (status === 'in_progress') {
    return (
      <span
        className="border-border bg-card flex size-6 shrink-0 items-center justify-center rounded-full border shadow-[0_0_0_4px_color-mix(in_oklch,var(--primary)_14%,transparent)] motion-safe:transition-all motion-safe:duration-300"
        aria-hidden="true"
      >
