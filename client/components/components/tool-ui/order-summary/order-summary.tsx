import { CheckCircle, Package } from '@/lib/matrix-icons'
...
function OrderSummaryRoot({
  ...
  if (isMalformedPayload) {
    return (
      ...
      >
        <div className="text-card-foreground bg-card rounded-lg border p-4 shadow-sm">
      ...
      >
        <div className={cn('space-y-4 p-4', isReceipt && 'opacity-95')}>
          <div>
            ...
            {isReceipt && choice && (
              <ReceiptBadge orderId={choice.orderId} confirmedAt={choice.confirmedAt} />
            )}
          </div>
