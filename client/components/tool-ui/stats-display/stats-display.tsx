    ...
    >
      {hasSparkline && (
        <Sparkline
          ...
          showFill
          fillOpacity={0.09}
          className="animate-in fade-in slide-in-from-bottom-12 fill-mode-both pointer-events-none absolute inset-x-0 top-2 bottom-2 duration-500 ease-[cubic-bezier(0.16,1,0.3,1)]"
          style={{ animationDelay: `${baseDelay}ms` }}
        />
      )}
      <span
        className="text-muted-foreground animate-in fade-in slide-in-from-bottom-1 fill-mode-both relative text-xs font-normal tracking-wider uppercase opacity-90 duration-500 ease-[cubic-bezier(0.16,1,0.3,1)]"
        style={{ animationDelay: `${baseDelay + 75}ms` }}
      >
        {stat.label}
      </span>
      <div
        className="animate-in fade-in slide-in-from-bottom-2 fill-mode-both relative flex items-baseline gap-2 pb-2 duration-500 ease-[cubic-bezier(0.16,1,0.3,1)]"
...
export function StatsDisplay({
  ...
  return (
    <article
      ...
      data-tool-ui-id={id}
      className={cn('w-full max-w-xl min-w-80', isSingle && 'max-w-sm', className)}
    >
