...
import { Check, X, Loader2, Timer, AlertCircle } from '@/lib/matrix-icons'
import type { LucideIcon } from '@/lib/matrix-icons'
...
function StepIndicator({ status }: StepIndicatorProps) {
  ...
  if (status === 'in-progress') {
    return (
      <span
        className="bg-card border-border flex size-6 shrink-0 items-center justify-center rounded-full border shadow-[0_0_0_4px_color-mix(in_oklch,var(--primary)_14%,transparent)] motion-safe:transition-all motion-safe:duration-300"
        aria-hidden="true"
      >
  ...
  if (status === 'completed') {
    return (
      <span
        className="bg-primary text-primary-foreground border-primary motion-safe:animate-in motion-safe:fade-in motion-safe:zoom-in-75 flex size-6 shrink-0 items-center justify-center rounded-full border shadow-sm motion-safe:duration-300 motion-safe:ease-out"
        aria-hidden="true"
      >
        <Check
          className="motion-safe:animate-in motion-safe:fade-in motion-safe:zoom-in-75 motion-safe:fill-mode-both size-4 motion-safe:delay-75 motion-safe:duration-200"
          strokeWidth={3}
        />
      </span>
    )
  ...
  if (status === 'failed') {
    return (
      <span
        className="bg-destructive border-destructive motion-safe:animate-in motion-safe:fade-in motion-safe:zoom-in-75 flex size-6 shrink-0 items-center justify-center rounded-full border text-white shadow-sm motion-safe:duration-300 motion-safe:ease-out dark:border-red-600 dark:bg-red-600"
        aria-hidden="true"
      >
        <X
          className="motion-safe:animate-in motion-safe:fade-in motion-safe:zoom-in-75 motion-safe:fill-mode-both size-4 motion-safe:delay-75 motion-safe:duration-200"
...
/**
 * ProgressSteps — the live step list on its own (no card chrome), so it can
 * be embedded inside an existing surface (e.g. a run card) without nesting
 * two `bg-card` layers. Mirrors the list rendered by ProgressTracker.Live.
 */
