...
export function DojoDesktopScreen({
  ...
  const shown: NeoDojo = effective ?? { state: 'off', conversationId: conv }
  ...
  const run = async (
    ...
    try {
      ...
      // Server truth is unchanged on failure; the button simply re-enables
  ...
  return (
    <NeoDesktop
      ...
      powerBusy={powerBusy}
      onBoot={conv ? () => void run(setPowerBusy, dojoBoot) : undefined}
