...
import type { ChatPhase, NeoFinanceEvent, NeoStep, NeoTask } from '@/hooks/api/useChat'
...
import {
  ...
  financeEventLabel,
} from '@/components/matrix/finance/finance-computer-screen'
...
function buildScreens(
  ...
  phase: ChatPhase,
  showMedia: boolean,
  financeLabel: (event: NeoFinanceEvent) => string,
  ...
  for (const event of task.finance ?? []) {
    list.push({
      id: `finance-${event.id}`,
      label: financeLabel(event),
      icon: Monitor,
      running: false,
      node: <FinanceComputerScreen event={event} />,
    })
  }
...
export function NeoComputer({
  ...
  const financeLabel = (event: NeoFinanceEvent) =>
    financeEventLabel(event, {
      quote: tFinance('quoteLabel'),
      chart: tFinance('chartLabel'),
      movers: tFinance('movers'),
      sectors: tFinance('sectors'),
      news: tFinance('news'),
      status: tFinance('status'),
      economicData: tFinance('economicData'),
      markets: tFinance('title'),
    })
  ...
  return (
    <motion.div
      ...
      transition={reduce ? { duration: 0 } : motionTransition.page}
      className={cn(
    ...
    >
      ...
      <div className="flex shrink-0 items-center gap-3 px-4 py-3.5">
        <AnimatePresence initial={false} mode="popLayout">
          <motion.span
            ...
            className="bg-primary/15 text-primary grid size-8 shrink-0 place-items-center rounded-[0.7rem]"
          >
      ...
      <div ref={bodyRef} className="min-h-0 flex-1 overflow-x-hidden overflow-y-auto px-4 pb-4">
        {active ? (
          <AnimatePresence initial={false} mode="popLayout">
            <motion.div
              data-computer-screen={active.id}
              key={active.id}
              layout
              initial={reduce ? false : contentMotion.initial}
              animate={contentMotion.animate}
