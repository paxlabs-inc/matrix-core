...
export function NeoMemoryCard({ memory }: { memory?: NeoMemory }) {
  ...
  const { storySoFar, timeline } = memory
    ...
    <div className="flex flex-col gap-3">
      {storySoFar ? (
        <div className="bg-card flex flex-col gap-2 rounded-xl p-3.5">
          <div className="flex items-center gap-2">
            ...
            <p className="text-foreground text-sm font-bold">What Neo is carrying forward</p>
          </div>
      ...
      {timeline.length > 0 ? (
        <div className="bg-card flex flex-col gap-2.5 rounded-xl p-3.5">
          <p className="text-foreground px-0.5 text-sm font-bold">Timeline so far</p>
