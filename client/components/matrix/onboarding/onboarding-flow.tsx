  ...
  return (
    ...
    >
      <Layout
        ...
        content={
          <LayoutContent className="mx-auto flex w-full max-w-md flex-col justify-center">
              ...
              <div className="flex h-full min-h-[36rem] flex-col">
                <div className="mb-8 flex items-center gap-4">
                  ...
                  {currentIdx < totalSteps ? (
                    <ProgressBar
                      value={currentIdx + 1}
                      max={totalSteps}
                      label={`Onboarding step ${currentIdx + 1} of ${totalSteps}`}
                      isLabelHidden
                      className="flex-1"
                    />
                  ) : null}
                ...
                {isFailed ? (
                  <Text
                    ...
                    className="mt-4"
                  >
                    Your agent runtime couldn&apos;t start. You can continue and retry from
                    Settings.
                  </Text>
                ) : null}
              </div>
...
function ReadinessIndicator({ state }: { state: string }) {
  ...
  return (
    <Badge
      variant={state === 'active' ? 'success' : state === 'failed' ? 'error' : 'info'}
      label={label}
    />
  )
}
