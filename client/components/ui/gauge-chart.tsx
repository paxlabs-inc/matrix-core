...
export function GaugeChart(props: GaugeChartProps) {
  ...
  const color = pct >= danger ? '#f87171' : pct >= warning ? '#fbbf24' : '#34d399'
  ...
  if (variant === 'text') {
    ...
    return (
      <div className="bg-card text-card-foreground rounded-2xl border p-4 text-center">
  ...
  if (variant === 'stacked') {
    ...
    const stackData = [{ primary: props.value, secondary: props.secondValue ?? 0 }]
  ...
  return (
    <div
      className="bg-card text-card-foreground rounded-2xl border p-4 text-center"
      style={{ width: 200 }}
    >
      <ChartContainer config={gaugeConfig} className="mx-auto h-[160px] w-[160px]">
        <RadialBarChart
          data={gaugeData}
          startAngle={0}
          endAngle={valueAngle}
