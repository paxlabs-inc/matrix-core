...
- [ ] 6. Reduce Neo to append + activate + render + transport
  - [ ] 6.1 Wire the Neo turn loop to cortex.AppendMessage
    - In agents/neo/internal/agent/agent.go, append each user / assistant / tool message to cortex via AppendMessage as the turn progresses, making cortex the owner of the transcript instead of the in-memory working slice being the source of truth
    - _Requirements: 9.1_
  ...
  - [ ] 6.3 Wire the recursive memory_recall tool to cortex recursive recall
    - Point agents/neo/internal/tools memory_recall (RecallFunc, tools.go:132) at the cortex recursive-recall surface (req.8), preserving the AmbientRetrievalTopK pull-over-push philosophy and the as_of parameter
