...
See the full design below (system map, the capture extraction class, the cortex opportunity queue + lifecycle, the three-layer non-financial gating with the restricted-tool-surface structural guarantee, the AUTOMATRIX Chronos alarm with randomized jitter + busy-check + opt-in re-check, the supervised restricted-surface execution path, the pluggable ntfy/Apprise Notifier + durable in-app record, the opt-in control surface, config knobs, non-goals, and the no-fakes verification strategy). Authored as a rich markdown fragment (design.body.md) and inlined here verbatim, because diagrams and tables do not fit single-line kvx values.
