...
// API, used by codyd to turn a finished project into an on-demand preview.
//
...
//
// codyd-facing seam: the Client interface (Create/Exec/WriteFiles/Destroy).
...
// Sandbox statuses Railway reports (subset codyd dispatches on). The exact
...
// Sentinel errors. Callers use errors.Is to dispatch; codyd maps them onto its
...
// Sandbox is the subset of Railway's sandbox object codyd cares about.
...
// the aggregated result once the command completes — the value codyd needs for
...
// Client is the codyd-facing seam. Task 3.3 (preview lifecycle) consumes ONLY
...
// codyd caller (not by this package). Endpoint and HTTPClient are override
