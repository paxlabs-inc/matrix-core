...
// Package preview implements Cody's "preview is a deliverable" lifecycle: on
...
// Event type names (mirrored in the codyd trace whitelist and the client
...
const (
	...
	// defaultImage carries a Node runtime because most Cody previews are web
	// apps (npm install + dev server). It has apt on top of Debian bookworm so a
	// preview can bootstrap python/go on demand. Override with CODY_PREVIEW_IMAGE
...
type Config struct {
	...
	// and the value codyd registers its target under. MUST match the JWT subject.
	UserID string
	// RouterInternalURL is the base of the router's internal listener, e.g.
	// http://matrix-router.railway.internal:8088 . codyd registers/deregisters
...
// Manager owns the live previews for one codyd (one user). It is safe for
// concurrent use.
type Manager struct {
	sb   sandbox.Client
	emit Emit
...
// synchronous; the engine calls it from a goroutine on plan completion or from
// the re-preview route.
...
func (m *Manager) provision(ctx context.Context, req Request) (Info, error) {
	model, err := workspace.LoadOrScan(req.Root)
	if err != nil {
		return Info{}, fmt.Errorf("scan project: %w", err)
	}
	spec, ok := DetectStart(model)
	if !ok {
	...
	start := fmt.Sprintf("nohup %s > /tmp/cody-preview.log 2>&1 & echo started", spec.Serve)
...
// Close destroys every live preview (graceful codyd shutdown).
...
var skipDir = map[string]bool{
	...
	".venv": true, "venv": true, ".cache": true, ".turbo": true, ".cody": true,
}
