...
func (c *Client) Chat(ctx context.Context, req ChatRequest) (*ChatResult, error) {
	// The fleet id is xAI's native model code ("grok-build-0.1", "grok-4.3"),
	// so no send-time model rewrite is needed on the direct hop (unlike the
	...
	// xAI's grok-4.3 accepts `reasoning_effort` — pin it explicitly from the
	// per-client flag ("high" when set, "none" when clear) so token-tight roles
...
// server split it into the separate reasoning_content channel. Grok (grok-*)
// uses xAI's native `reasoning_effort` field instead (see reasoningEffort).
// Returns nil for models that reason by default (never sent).
