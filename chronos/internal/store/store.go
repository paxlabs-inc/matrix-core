// Package store provides Postgres access and the migration runner for chronosd.
// The alarms table IS the durable timer (invariant i1): state lives in Postgres,
// never in memory, so a restart never loses a scheduled wake.
package store
...
// dedicated chronos_schema_migrations ledger (Chronos shares the matrix DB with
// router + gateway, so it must NOT collide with their migration tables).
func (s *Store) Migrate(ctx context.Context, dir string) error {
	...
	for _, name := range files {
		...
		if _, err := tx.Exec(ctx, `INSERT INTO chronos_schema_migrations (version) VALUES ($1)`, name); err != nil {
			_ = tx.Rollback(ctx)
...
func (s *Store) ensureMigrationTable(ctx context.Context) error {
	_, err := s.pool.Exec(ctx, `
		CREATE TABLE IF NOT EXISTS chronos_schema_migrations (
			version TEXT PRIMARY KEY,
			applied_at TIMESTAMPTZ NOT NULL DEFAULT now()
		)`)
	if err != nil {
		return fmt.Errorf("store: ensure chronos_schema_migrations: %w", err)
	}
	return nil
}
...
func (s *Store) isApplied(ctx context.Context, version string) (bool, error) {
	var n int
	err := s.pool.QueryRow(ctx, `SELECT COUNT(1) FROM chronos_schema_migrations WHERE version = $1`, version).Scan(&n)
	if err != nil {
