-- 002_credit_ledger.sql — sess#32 ambient-architect plan §5.15
--
-- Mirrors gateway/migrations/001_credit_ledger.sql verbatim so the
-- box-level migration runner (apply numbered files in deploy/box/
-- postgres/migrations/) sees the credit_ledger schema. The gateway
-- module ships its OWN copy at gateway/migrations/001_credit_ledger.sql
-- so its installer can run psql -f without depending on the box repo
-- layout. Both files MUST stay in sync; CI fmt-check should diff
-- them on every change. Single source of truth lives under gateway/.
--
