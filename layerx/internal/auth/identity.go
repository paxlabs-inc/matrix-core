// Package auth implements layerxd's two principal-auth primitives: the
// agent-DID ed25519 challenge/verify lane (proves WHICH agent owns the balance)
// and the short-lived HMAC principal token minted on a successful verify.
//
// This mirrors chronos/uwac identity: the daemon's executor key IS the agent
// identity, and the DID label IS the owner's Supabase user UUID
// (did:matrix:<MATRIX_USER_ID>:<keyfp>). layerxd owns BOTH sides of its own
// challenge (layerx.mjs signs, layerxd verifies), so the challenge message
...
// MUST stay in lockstep with tools/layerx/layerx.mjs.
func ChallengeMessage(did, nonce string) string {
	return "matrix-layerx-auth:" + did + ":" + nonce
}
...
func (c *Challenges) Create(did string) (nonce, message string) {
	b := make([]byte, 24)
	// crypto/rand failure is catastrophic and unrecoverable — never emit a
	// predictable nonce (it would let a replayed signature pass verify).
	if _, err := io.ReadFull(rand.Reader, b); err != nil {
		panic("layerx/auth: crypto/rand unavailable: " + err.Error())
	}
