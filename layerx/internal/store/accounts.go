...
func (s *Store) Pay(ctx context.Context, fromDID, toDID string, amountMicro int64, tier string,
	...
	// Pre-allocate the monotonic seq + timestamp so the Merkle leaf + sequencer
	// signature are computed BEFORE the insert and written in ONE statement. The
	// prior INSERT-then-UPDATE produced a dead tuple per payment (millions/day =
	// punishing autovacuum churn on the hottest table). nextval() works on the
	// identity's underlying sequence; the seq column is GENERATED BY DEFAULT
	// (migration 005) so an explicit value is accepted.
	var seq int64
	if err := tx.QueryRow(ctx, `SELECT nextval(pg_get_serial_sequence('transfers', 'seq'))`).Scan(&seq); err != nil {
		return PayResult{}, fmt.Errorf("store: allocate seq: %w", err)
	}
	ts := time.Now().UTC()
