...
See the full design below (overview, hard constraints, architecture with sequence/flow diagrams, the Surface-State Model, components and interfaces, data models, formal function specs, and pseudocode). The design content is authored as a rich markdown fragment (design.body.md) and inlined here verbatim, because mermaid diagrams and code blocks do not fit single-line kvx values.
