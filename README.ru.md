...
Matrix Core — открытый исходный код, и вы можете **свободно форкать и изменять его**. Однако ветку `main` разрабатывает строго основная команда: непрошеные pull request'ы, как правило, не принимаются, а внешние изменения мы берём только после того, как поработали с автором напрямую и поняли качество его работы.

Прежде чем что-либо открывать, прочитайте политику участия в начале [Руководства для участников](./CONTRIBUTING.md). Issue, сообщения об ошибках и раскрытие уязвимостей всегда приветствуются.
