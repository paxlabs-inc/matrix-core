...
// core/mcl/llm/model.go — Centra AI Model Router (Session 31a, 2026-05-27).
//
...
// core/mcl/cmd/mclc, bridge/cmd/mclc-cortex, executor/cmd/mcl-execute,
...
// Session 31a model router design. Models pinned to xAI for the primary path
// (grok-4.3 via api.x.ai; matches core/mcl/llm provider detection, which resolves
// the bare "grok-*" shape to xAI). Together/Fireworks/Baseten remain reachable
// as explicit alt paths the gateway may select on 5xx or latency-budget breach.
//
// Provider-specific model ID conventions:
//
//	xAI:       "grok-*"           (e.g. "grok-4.3")
//	Baseten:   "<vendor>/<name>"  (e.g. "moonshotai/Kimi-K2")
...
// core/mcl/ir/plan.go with recursive $defs/plan_node.
func DefaultGrammars() map[string]*GrammarDef {
	return map[string]*GrammarDef{
		...
		// Mirrors core/mcl/ir/plan.go: PlanTree + PlanNode with ValidNodeKinds +
