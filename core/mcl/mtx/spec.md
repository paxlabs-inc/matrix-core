...
- A skill's compile-time procedure is a `.mtx` file (`protocol/skills/<slug>/SKILL.mtx`).
- The compiler's own grammar, verb rules, and pipeline wiring are `.mtx` files (`core/mcl/core/`).
...
## 12. Compiler-core `.mtx` files (`core/mcl/core/`)
...
`protocol/skills/writing-plans/SKILL.mtx` — see `core/mcl/core/` for the reference core files.
...
`core/mcl/mtx/` includes a `vscode/` sub-directory with a TextMate grammar (`.tmLanguage.json`) that forks the `.kvx` grammar and adds:
